...
class Octosuite:
    ...
    def get_user_profile(self, args):
        ...
        if response[0] == 404:
            ...
            for attr in self.__attribute.user_profile_attributes()[0]:
                user_profile_tree.add(f"{self.__attribute.user_profile_attributes()[1][attr]}: {response[1][attr]}")
    ...
    def get_repository_profile(self, args):
        ...
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=username))
        elif response[0] == 200:
    ...
    def get_repository_path_contents(self, args):
        ...
        if response[0] == 404:
            xprint(self.__message.information_not_found(param1=repository, param2=path_name, param3=repository_owner))
        elif response[0] == 200:
            for content_count, content in enumerate(response[1], start=1):
                ...
                for attr in self.__attribute.path_attributes()[0]:
                    path_contents_tree.add(f"{self.__attribute.path_attributes()[1][attr]}: {content[attr]}")
    ...
    def get_repository_contributors(self, args):
        ...
        limit = args.limit or Prompt.ask(self.__message.limit_output(title="Repository contributors"))

        response = send_request(f"{self.__endpoint}/repos/{repository_owner}/"
                                f"{repository}/contributors?per_page={limit}")
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=repository_owner))
        elif response[0] == 200:
            for contributor in response[1]:
                contributor_tree = Tree(f"\n{contributor['login']}")
                for attr in self.__attribute.user_summary_attributes()[0]:
                    contributor_tree.add(f"{self.__attribute.user_summary_attributes()[1][attr]}: {contributor[attr]}")
                xprint(contributor_tree)

                if args.log_to_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_repo_contributors(contributor=contributor, repo_name=repository)
            else:
                xprint(response[1])
    ...
    def get_repository_stargazers(self, args):
        ...
        limit = args.limit or Prompt.ask(self.__message.limit_output(title="Repository stargazers"))

        response = send_request(f"{self.__endpoint}/repos/{repository_owner}/{repository}/stargazers?per_page={limit}")
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=repository_owner))
        elif response[0] == {}:
            xprint(self.__message.repo_does_not_have(title="stargazers", repository=repository,
                                                     username=repository_owner))
        elif response[0] == 200:
            for stargazer in response[1]:
                stargazer_tree = Tree(f"\n{stargazer['login']}")
                for attr in self.__attribute.user_summary_attributes()[0]:
                    stargazer_tree.add(f"{self.__attribute.user_summary_attributes()[1][attr]}: {stargazer[attr]}")
                xprint(stargazer_tree)

                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_repo_stargazers(stargazer=stargazer, repo_name=repository)
        else:
            xprint(response[1])
    ...
    def get_repository_forks(self, args):
        repository = args.repository or Prompt.ask(f"{self.__emoji.FILE_CABINET} (repository)")
        repository_owner = args.username or Prompt.ask("-> (@Owner)")
        limit = args.limit or Prompt.ask(self.__message.limit_output(title="Repository forks"))

        response = send_request(f"{self.__endpoint}/repos/{repository_owner}/{repository}/forks?per_page={limit}")
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=repository_owner))
        elif response[0] == {}:
            xprint(self.__message.repo_does_not_have(title="forks", repository=repository, username=repository_owner))
        elif response[0] == 200:
            for count, fork in enumerate(response):
                fork_tree = Tree(f"\n{fork['full_name']}")
                for attr in self.__attribute.repository_profile_attributes()[0]:
                    fork_tree.add(f"{self.__attribute.repository_profile_attributes()[1][attr]}: {fork[attr]}")
                xprint(fork_tree)
    ...
    def get_repository_issues(self, args):
        repository = args.repository or Prompt.ask(f"{self.__emoji.FILE_CABINET} (repository)")
        repository_owner = args.username or Prompt.ask("-> (@Owner)")
        limit = args.limit or Prompt.ask(self.__message.limit_output(title="Repository Issues"))

        response = send_request(f"{self.__endpoint}/repos/{repository_owner}/{repository}/issues?per_page={limit}")
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=repository_owner))
        elif not response[1]:
            xprint(self.__message.repo_does_not_have(title="open issues", repository=repository,
                                                     username=repository_owner))
        elif response[0] == 200:
            for issue in response[1]:
                issues_tree = Tree(f"\n{issue['title']}")
                for attr in self.__attribute.repository_issues_attributes()[0]:
                    issues_tree.add(f"{self.__attribute.repository_issues_attributes()[1][attr]}: {issue[attr]}")
                xprint(issues_tree)
                xprint(issue['body'])
                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
    ...
    def get_repository_releases(self, args):
        ...
        limit = args.limit or Prompt.ask(self.__message.limit_output(title="Repository Releases"))

        response = send_request(f"{self.__endpoint}/repos/{repository_owner}/{repository}/releases?per_page={limit}")
        if response[0] == 404:
            xprint(self.__message.repo_or_user_not_found(repository=repository, username=repository_owner))
        elif not response[0]:
            xprint(self.__message.repo_does_not_have(title="releases", repository=repository,
                                                     username=repository_owner))
        elif response[0] == 200:
            for release in response[1]:
                releases_tree = Tree(f"\n{release['name']}")
                for attr in self.__attribute.repo_releases_attributes()[0]:
                    releases_tree.add(f"{self.__attribute.repo_releases_attributes()[1][attr]}: {release[attr]}")
                xprint(releases_tree)
                xprint(release['body'])

                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_repo_releases(release=release, repo_name=repository)
        else:
            xprint(response[1])
    ...
    def get_organisation_repositories(self, args):
        ...
        if response[0] == 404:
            ...
            for repository in response[1]:
                ...
                for attr in self.__attribute.repository_profile_attributes()[0]:
                    repos_tree.add(f"{self.__attribute.repository_profile_attributes()[1][attr]}: {repository[attr]}")
                xprint(repos_tree)

                if args.log_csv or Prompt.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_organisation_repos(repository=repository, organisation=organisation)
        else:
            xprint(response[1])
    ...
    def get_organisation_events(self, args):
        ...
        if response[0] == 404:
            ...
            for event in response[1]:
                ...
                self.__logger.log_organisation_events(event=event, organisation=organisation)
        else:
            xprint(response[1])
    ...
    def is_organisation_member(self, args):
        ...
        if response[0] == 204:
            xprint(f"@{username} (user) is a public member of @{organisation} (organisation)")
        else:
    ...
    def get_user_repositories(self, args):
        ...
        if response[0] == 404:
            ...
            for repository in response[1]:
                ...
                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_user_repositories(repository=repository, username=username)
        else:
            xprint(response[1])
        ...
        if not response[1]:
            ...
            for gist in response[1]:
                ...
                for attr in self.__attribute.gist_attributes()[0]:
                    gists_tree.add(f"{self.__attribute.gist_attributes()[1][attr]}: {gist[attr]}")
                xprint(gists_tree)
        ...
        if not response[1]:
            ...
            for organisation in response[1]:
                ...
                for attr in self.__attribute.user_orgs_information_attributes()[0]:
                    org_tree.add(f"{self.__attribute.user_orgs_information_attributes()[1][attr]}: "
                                 f"{organisation[attr]}")
                xprint(org_tree)

                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_user_organisations(organisation=organisation, username=username)
        else:
            xprint(response[1])
    ...
    def get_user_subscriptions(self, args):
        ...
        if not response[1]:
            ...
            for repository in response[1]:
                ...
                for attr in self.__attribute.repository_profile_attributes()[0]:
                    subscriptions_tree.add(f"{self.__attribute.repository_profile_attributes()[1][attr]}: "
                                           f"{repository[attr]}")
                xprint(subscriptions_tree)

                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_user_subscriptions(repository=repository, username=username)
        else:
            xprint(response[1])
    ...
    def get_user_following(self, args):
        ...
        if not response[1]:
            ...
            for user in response[1]:
                ...
                for attr in self.__attribute.user_summary_attributes()[0]:
                    following_tree.add(f"{self.__attribute.user_summary_attributes()[1][attr]}: {user[attr]}")
    ...
    def get_user_followers(self, args):
        ...
        if not response[1]:
            ...
            for follower in response[1]:
                ...
                for attr in self.__attribute.user_summary_attributes()[0]:
                    followers_tree.add(f"{self.__attribute.user_summary_attributes()[1][attr]}: {follower[attr]}")
                xprint(followers_tree)

                if args.log_csv or Confirm.ask(self.__message.prompt_log_csv()):
                    self.__logger.log_user_followers(follower=follower, username=username)
        else:
            xprint(response[1])
    ...
    def search_users(self, args):
        query = args.query or Prompt.ask(f"{self.__emoji.MAGNIFYING_GLASS_LEFT}{self.__emoji.MAGNIFYING_GLASS_LEFT}"
                                         f" (query)")
        limit = args.limit or Prompt.ask(self.__message.limit_output("User Search"))

        response = send_request(f"{self.__endpoint}/search/users?q={query}&per_page={limit}")
        for user in response[1]['items']:
            users_search_tree = Tree(f"\n{user['login']}")
            for attr in self.__attribute.user_summary_attributes()[0]:
                users_search_tree.add(f"{self.__attribute.user_summary_attributes()[1][attr]}: {user[attr]}")
    ...
    def search_repositories(self, args):
        query = args.query or Prompt.ask(f"{self.__emoji.FILE_CABINET}{self.__emoji.MAGNIFYING_GLASS_LEFT}"
                                         f" (repository) (search)")
        limit = args.limit or Prompt.ask(self.__message.limit_output("Repository Search"))

        response = send_request(f"{self.__endpoint}/search/repositories?q={query}&per_page={limit}")
        for repository in response[1]['items']:
            repos_search_tree = Tree(f"\n{repository['full_name']}")
            for attr in self.__attribute.repository_profile_attributes()[0]:
                repos_search_tree.add(f"{self.__attribute.repository_profile_attributes()[1][attr]}: "
                                      f"{repository[attr]}")
    ...
    def search_topics(self, args):
        ...
        response = send_request(f"{self.__endpoint}/search/topics?q={query}&per_page={limit}")
        for topic in response[1]['items']:
            topics_search_tree = Tree(f"\n{topic['name']}")
            for attr in self.__attribute.topic_information_attributes()[0]:
                topics_search_tree.add(f"{self.__attribute.topic_information_attributes()[1][attr]}: {topic[attr]}")
    ...
    def search_issues(self, args):
        ...
        response = send_request(f"{self.__endpoint}/search/issues?q={query}&per_page={limit}")
        for issue in response[1]['items']:
            issues_search_tree = Tree(f"\n{issue['title']}")
            for attr in self.__attribute.repository_issues_attributes()[0]:
                issues_search_tree.add(f"{self.__attribute.repository_issues_attributes()[1][attr]}: {issue[attr]}")
    ...
    def search_commits(self, args):
        ...
        response = send_request(f"{self.__endpoint}/search/commits?q={query}&per_page={limit}")
    ...
    @staticmethod
    def __get_email_from_contributor(username: str, repository: str, contributor: str) -> str:
        """
        ...
        response = send_request(commits_url, authentication=HTTPBasicAuth(username, ''))[1]
        ...
        commit_details = send_request(endpoint=commit_details_url, authentication=HTTPBasicAuth(username, ''))[1]
    ...
    def __get_unforked_repositories(self, username: str) -> list:
        ...
        response = send_request(endpoint=repos_url, authentication=HTTPBasicAuth(username, ''))
